...
1. **Security-First Design**:
   ...
   - Backend services operate behind NAT
   - Explicit allow/deny rules for all critical ports

2. **Environment-Aware Naming**:
   - All resources include environment prefix (e.g., `epamqa-nat-gw`)
   - Consistent tagging across resources

3. **Cost Optimization**:
   - Uses Standard SKU NAT Gateway (Basic not available)
   - Minimum idle timeout configuration
   - Free-tier eligible VM sizes where possible

4. **High Availability**:
   - NAT Gateway zonal deployment
   - Multiple backend instances provisioned

### Module Outputs

This module provides the following outputs for other modules to consume:
- Virtual Network ID
- Backend Subnet ID
- Database Subnet ID
